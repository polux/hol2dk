...
module Proofobjects : Proofobject_primitives = struct
  ...
  let export_thm out (thmname, p, c_opt) =
    match c_opt with
      | Some cl' ->
          let cl = term2nterm cl' in
          let fvcl = fv cl in
  ...
  (*          | [a;b] -> Narrow (hol_type2ntype a, hol_type2ntype b) *)
  (*          | _ -> failwith "hol_type2ntype: wrong number of arguments for fun") *)
  (*   | _ -> failwith "hol_type2ntype: HOL type not implemented yet" *)
  ...
  (* let type_ncst = function *)
  (*   | Nimp -> Narrow (Nbool, Narrow (Nbool, Nbool)) *)
  (*   | Nforall a -> Narrow (Narrow (a, Nbool), Nbool) *)
  (*   | Neq a -> Narrow (a, Narrow (a, Nbool)) *)


  (* type nterm = *)
  (*   | Nvar of int *)
  (*   | Nlam of int * ntype * nterm *)
  (*   | Napp of nterm * nterm *)
  (*   | Ncst of ncst *)


  (* let rec is_closed l = function *)
  (*   | Nvar i -> List.mem i l *)
  (*   | Nlam (i, _, t) -> is_closed (i::l) t *)
  (*   | Napp (u, v) -> is_closed u && is_closed v *)
  (*   | Ncst _ -> true *)


  (* let rec type_nterm l = function *)
  (*   | Nvar i -> List.assoc i l *)
  (*   | Nlam (i, ty, t) -> Narrow (ty, type_nterm ((i, ty)::l) t) *)
  (*   | Napp (u, v) -> *)
  (*       (match type_nterm l u with *)
  (*          | Narrow (_, b) -> b *)
  (*          | _ -> failwith "type_nterm: the first term of an application must have a functional type") *)
  (*   | Ncst c -> type_ncst c *)


  (* let rec term2nterm l = function *)
  (*   | Var (x, ty) -> List.assoc (x, hol_type2ntype ty) l *)
  (*   | Comb (t1, t2) -> *)
  (*       let (u1, ty) = term2nterm l t1 in *)
  (*       let (u2, _) = term2nterm l t2 in *)
  (*       (match ty with *)
  (*          | Narrow (_, a) -> (Napp (u1, u2), a) *)
  (*          | _ -> failwith "term2nterm: the type of the first term of an application must be functional") *)
  (*   | Abs (t1, t2) -> *)
  (*       (match t1 with *)
  (*          | Var (x, ty) -> *)
  (*              let ty' = hol_type2ntype ty in *)
  (*              (Nlam (fun y -> term2nterm (((x, ty'), y)::l) t2), *)
  (*               Narrow (ty',  *)


  (* (\* The type of proof trees in HOAS *\) *)

  (* type nproof = nproof_content * string list *)

  (* and nproof_content = *)
  (*   | Nprefl of term *)
  (*   (\* | Nprefl of term *\) *)
  (*   (\* | Nbeta of string * hol_type * term *\) *)
  (*   (\* | Ninstt of nproof * (string * hol_type) list *\) *)
  (*   (\* | Npabs of nproof * string * hol_type *\) *)
  (*   (\* | Ndisch of nproof * term *\) *)
  (*   (\* | Nhyp of term *\) *)
  (*   (\* | Nspec of nproof * term *\) *)
  (*   (\* | Ninst of nproof * (string * hol_type * term) list *\) *)
  (*   (\* | Ngen of nproof * string * hol_type *\) *)
  (*   (\* | Nsym of nproof *\) *)
  (*   (\* | Nptrans of nproof * nproof *\) *)
  (*   (\* | Ncomb of nproof * nproof *\) *)
  (*   (\* | Neqmp of nproof * nproof *\) *)
  (*   (\* | Nexists of nproof * term * term *\) *)
  (*   (\* | Nchoose of string * hol_type * nproof * nproof *\) *)
  (*   (\* | Nconj of nproof * nproof *\) *)
  (*   (\* | Nconjunct1 of nproof *\) *)
  (*   (\* | Nconjunct2 of nproof *\) *)
  (*   (\* | Ndisj1 of nproof * term *\) *)
  (*   (\* | Ndisj2 of nproof * term *\) *)
  (*   (\* | Ndisjcases of nproof * nproof * nproof *\) *)
  (*   (\* | Nnoti of nproof *\) *)
  (*   (\* | Nnote of nproof *\) *)
  (*   (\* | Ncontr of nproof * term *\) *)
  (*   (\* | Nimpas of nproof * nproof *\) *)
  (*   (\* | Naxm of string * term *\) *)
  (*   (\* | Ndef of string * hol_type * term *\) *)
  (*   (\* | Ntyintro of hol_type * string * hol_type list * string * string * term *\) *)
