...
and reloc_patt floc sh =
  self where rec self =
    ...
    | PaLab loc x2 ->
        let loc = floc loc in
        PaLab loc (vala_map (List.map (fun (y1,y2) -> (self y1, vala_map (option_map self) y2))) x2)
    (* | PaLab loc x1 x2 -> *)
    (*     let loc = floc loc in *)
...
and reloc_expr floc sh =
  self where rec self =
    ...
    | ExLab loc x1 ->
        let loc = floc loc in
        ExLab loc (vala_map (List.map (fun (y1,y2) -> (reloc_patt floc sh y1, vala_map (option_map self) y2))) x1)
    (* | ExLab loc x1 x2 -> *)
    (*     let loc = floc loc in *)
